...
> **Note:** the `terraform apply` examples below predate the Terragrunt
> catalog and describe this module in isolation. In `terraform/aws/live/`,
> every unit (including `security-rules`) is generated by
> [`catalog/stacks/internal`](../../catalog/stacks/internal/README.md) and
> applied with `terragrunt apply` / `terragrunt run-all apply`, not bare
> `terraform` — the directory paths below are otherwise still accurate.
