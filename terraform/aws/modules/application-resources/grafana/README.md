...
| <a name="input_database_master_password_secretsmanager_secret_id"></a> [database\_master\_password\_secretsmanager\_secret\_id](#input\_database\_master\_password\_secretsmanager\_secret\_id) | ARN or name of an existing AWS Secrets Manager secret to fetch the master DB password from. Takes precedence over database\_master\_password. Cannot be used together with database\_manage\_master\_user\_password = true | `string` | `null` | no |
| <a name="input_database_master_password_secretsmanager_secret_key"></a> [database\_master\_password\_secretsmanager\_secret\_key](#input\_database\_master\_password\_secretsmanager\_secret\_key) | JSON key within the Secrets Manager secret that holds the master password (e.g. "password"). If null, the raw secret string is used | `string` | `null` | no |
