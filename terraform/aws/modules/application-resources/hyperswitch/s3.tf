...
# =========================================================================
# S3 BUCKET - Card Data Migration
# =========================================================================

locals {
  s3_card_data_migration_bucket_name = try(var.s3_card_data_migration.bucket_name, null) != null ? var.s3_card_data_migration.bucket_name : "${local.name_prefix}-card-data-migration"
}

module "s3_card_data_migration" {
  source  = "terraform-aws-modules/s3-bucket/aws"
  version = "~> 5.0"

  count = local.s3_card_data_migration_create ? 1 : 0

  bucket        = local.s3_card_data_migration_bucket_name
  force_destroy = try(var.s3_card_data_migration.force_destroy, false)

  # Versioning
  versioning = {
    enabled = try(var.s3_card_data_migration.versioning_enabled, true)
  }
