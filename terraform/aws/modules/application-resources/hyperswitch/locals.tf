locals {
  ...
  # S3 Card Data Migration feature
  s3_card_data_migration_enabled    = var.s3_card_data_migration != {} && (try(var.s3_card_data_migration.create, false) || try(var.s3_card_data_migration.bucket_arn, null) != null)
  s3_card_data_migration_create     = try(var.s3_card_data_migration.create, false)
  s3_card_data_migration_bucket_arn = local.s3_card_data_migration_create ? (length(module.s3_card_data_migration) > 0 ? module.s3_card_data_migration[0].s3_bucket_arn : null) : try(var.s3_card_data_migration.bucket_arn, null)
