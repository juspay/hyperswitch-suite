...
variable "s3" {
  description = "S3 bucket configuration for Wazuh logs storage. Set to {} to disable. Set create=true to create bucket, or create=false with bucket_arn to use existing."
  type = object({
...
variable "cloudwatch_logs_policy" {
  description = "JSON-encoded IAM policy granting CloudWatch Logs read access (e.g. for RDS log groups). If null, no CloudWatch Logs policy is created."
  type        = string
  default     = null
}
