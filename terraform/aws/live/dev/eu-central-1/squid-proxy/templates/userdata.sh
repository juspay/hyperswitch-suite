...
sudo aws s3 cp "s3://${BUCKET_NAME}/${S3_PREFIX}/wazuh.conf" "/var/ossec/etc/ossec.conf" || echo "Wazuh config not found, skipping..."
...
sudo aws s3 cp "s3://${BUCKET_NAME}/${S3_PREFIX}/squid_vector.toml" "/etc/vector/vector.toml" || echo "Vector config not found, skipping..."
