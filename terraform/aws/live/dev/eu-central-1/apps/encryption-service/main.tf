locals {
      ...
      service_accounts = [
        {
          ...
          condition_value = [
            "system:serviceaccount:encryption-service-sandbox:encryption-service-role"
          ]
        }
      ]
    }
  ...
  # Modify to restrict access to specific KMS keys in production.

  inline_policies = {
    keymanager_kms = jsonencode({
      Version = "2012-10-17"
      Statement = [
        {
          Sid    = "VisualEditor0"
