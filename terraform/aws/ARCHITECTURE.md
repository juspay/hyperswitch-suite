...
Environment-specific deployments in `live/` directory, generated by
[Terragrunt Stacks](../catalog/stacks/internal/README.md) from the catalog.
**This is where Terragrunt commands run** — `live/` itself is generated and
committed; the source of truth is `catalog/stacks/internal` plus
`live/terragrunt.stack.hcl`'s per-environment `values`.

### Environments

| Environment | State Backend | Use Case |
|-------------|---------------|----------|
| `dev/` | S3 | Development, testing, reference |
| `pre-prod/` | S3 | Pre-production validation |
| `prod/` | S3 | Production workloads |

### Units (`live/<env>/eu-central-1/`)

Every environment renders the same 26 units from
[`catalog/stacks/internal`](../catalog/stacks/internal/README.md); only the
...
| `route53/` | `composition/route53` | Public + internal DNS zones | `vpc-network` |
| `acm/` | `composition/acm` | TLS certificates | `route53` |
| `database/` | `composition/database` | Aurora PostgreSQL | `vpc-network` |
| `elasticache/` | `composition/elasticache` | Redis-compatible cache | `vpc-network` |
| `efs/` | `composition/efs` | Shared file storage | `vpc-network` |
| `kafka/` | `composition/kafka` | Event streaming | `vpc-network` |

#### Proxy & Access

| Unit | Module | Purpose | Dependencies |
|-----------|--------|---------|--------------|
| `locker/` | `composition/locker` | Card vault service | `vpc-network` |
| `squid-proxy/` | `composition/squid-proxy` | Outbound proxy | `vpc-network` |
| `envoy-proxy/` | `composition/envoy-proxy` | Ingress proxy | `vpc-network`, `acm` |
...
| Unit | Module | Purpose | Dependencies |
|-----------|--------|---------|--------------|
| `application-stack/eks-01/` | `composition/eks` | EKS cluster + node groups | `vpc-network` |
| `application-stack/eks-resources/` | `composition/eks-kubernetes-resources` | RBAC, storage, autoscaler | `eks-01`, `efs` |
| `application-stack/utils-load-balancer/` | `composition/load-balancer` | Shared internal ALB | `vpc-network`, `eks-01` |

#### Apps (Kubernetes Workloads)

| Unit | Purpose | Dependencies |
|-----------|---------|--------------|
| `application-stack/apps/alb-controller/` | AWS LB Controller | `eks-01` |
| `application-stack/apps/external-secrets/` | Secrets management | `eks-01` |
| `application-stack/apps/istio/` | Service mesh | `eks-01`, `vpc-network` |
| `application-stack/apps/otel/` | Telemetry collector | `eks-01` |
| `application-stack/apps/vector-dr/` | Log/metric shipping | `eks-01` |
| `application-stack/apps/loki/` | Log aggregation | `eks-01`, `vpc-network` |
| `application-stack/apps/grafana/` | Monitoring dashboards | `eks-01`, `vpc-network` |
| `application-stack/apps/ratelimiter/` | Rate limiting | `eks-01`, `vpc-network` |
| `application-stack/apps/hyperswitch/` | Main application resources | `eks-01` |
| `application-stack/apps/decision-engine/` | Routing engine | `eks-01`, `hyperswitch` |
| `application-stack/apps/superposition/` | Config service | `eks-01`, `hyperswitch`, `vpc-network` |

#### Security (deploy last)

| Unit | Module | Purpose | Dependencies |
|-----------|--------|---------|--------------|
| `security-rules/` | `composition/security-rules` | Cross-unit SG rules | All of the above |
