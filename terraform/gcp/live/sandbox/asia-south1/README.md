...
need, the edge proxies and the card vault's data tier — for the `sandbox`,
`dev`, `pre-prod` and `prod` environments. This stack creates its own VPC.
...
| File | Role |
|---|---|
| `terragrunt.stack.hcl` | the 18 `unit` blocks, their `path`s and the `values` each receives |
