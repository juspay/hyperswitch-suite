...
| <a name="input_grant_token_creator"></a> [grant\_token\_creator](#input\_grant\_token\_creator) | Whether to grant roles/iam.serviceAccountTokenCreator to the project's<br/>Pub/Sub service agent. Upstream defaults this to true and creates the<br/>binding unconditionally, which is a PROJECT-LEVEL IAM grant appearing on<br/>every topic this module creates.<br/><br/>It is only actually required for PUSH subscriptions that authenticate to<br/>their endpoint with an OIDC token - the service agent needs it to mint<br/>those tokens. A topic with only pull subscriptions does not need it, so<br/>set false there and keep the grant off the project. | `bool` | `true` | no |
